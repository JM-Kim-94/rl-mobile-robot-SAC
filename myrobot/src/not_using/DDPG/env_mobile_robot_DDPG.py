...
class Env():
    ...
    def getState(self, scan, ep):
        ...
        if ep<50: target_size = 1.2
        elif (ep>=50 and ep<100): target_size = 0.7
    ...
    def setReward(self, state, done, action):
        ...
        reward = distance_reward * angle_reward + obstacle_reward + ang_vel_reward
    ...
    def step(self, action, ep, step):
        #max_vel = 0.5
        #linear_vel = round(max_vel*exp(-abs(action)), 4) + 0.1
        
        vel_cmd = Twist()
        vel_cmd.linear.x  = 0.5  # linear_vel # action[0][1] # 0.5  
        vel_cmd.angular.z = action[0][0]  # action
        self.pub_cmd_vel.publish(vel_cmd)
        
        print("EP:", ep, " Step:", step, " Goal_x:",self.goal_x, "  Goal_y:",self.goal_y)
