...
from DDPG.env_mobile_robot_SAC_new import Env
...
class NormalNoise:        
    def ep_to_sigma_angular(self, episode):
        if        episode<300:  return round(-0.003*episode  + 1.0, 5)
        else:                   return 0.05
...
class CriticNetwork(nn.Module):
    def __init__(self, state_size, action_size, learning_rate, name, chkpt_dir='/save_DDPG_models/'):
        ...
        self.fc2 = nn.Linear(self.fc2_dim, 128)
        nn.init.kaiming_normal_(self.fc2.weight)
        self.bn2 = nn.BatchNorm1d(128)
...
class Agent(object):
    ...
    def choose_action(self, observation, episode):
        ...
        mu_prime[0][1] = np.clip(mu_prime[0][1], 0, 0.5)
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter('SAC_log/1226_DDPG')
    ...
    for episode in range(agent.EPISODE): 
        ...
        for t in range(agent.MAX_STEP_SIZE):
            ...
            s_next, r, done, arrv = env.step(a, episode)           
