...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        return out_mu * self.action_bound
    ...
    def ep_to_sigma(self,episode):
        if episode<200:    return round(-0.003*episode  + 0.7, 5)
        else:    return 0.05

    def get_policy_action(self, state, episode):
        self.eval()
        mu_a  = self.forward(state)
        std_a = self.ep_to_sigma(episode)
        mu_a  = mu_a.item()
    ...
    def Learn(self, log_old_policy_pdf, states, actions, advantages, episode):
        self.train()
        ...
        std = torch.FloatTensor(self.ep_to_sigma(episode))
    ...
    def train(self, max_episode_num):
        ...
        for ep in range(int(max_episode_num)):
            ...
            for t in range(self.MAX_STEP_SIZE):
                ...
                mu_old, std_old, a = self.actor.get_policy_action(convertToTensorInput(s, self.state_dim), ep)
                ...
                for _ in range(self.EPOCHS):
                    ...
                    self.actor.Learn(log_old_policy_pdfs, convertToTensorInput(states, self.state_dim, states.shape[0]),actions, gaes, ep)                    
                self.unpause_sim()
