...
def goal_def():
    ...
    while True:
        distance = []
        print("finding proper goal position...")
        x, y = np.random.randint(-48,48)/10, np.random.randint(-48,48)/10
        for i in range(len(obstacles)):
            distance.append(sqrt(pow(obstacles[i][0]-x,2)+pow(obstacles[i][1]-y,2)))
        if (min(distance) >= 1.1) and (not(x==0 and y==0)):
            break
...
class Env():
    ...
    def getState(self, scan, ep):
        ...
        if ep<100: target_size = 1.5
        elif (ep>=100 and ep<300): target_size = 1.0
    ...
    def setReward(self, state, done, action):
        ...
        if (obstacle_min_range < 0.6):
            obstacle_reward = -25/(obstacle_min_range+0.0001)
        else:
            obstacle_reward = 0.0
            ang_vel_rate = abs(action)/1.5
        #obstacle_reward = 0.0
    ...
    def step(self, action, ep):
        ...
        max_lin_vel = 0.6
        min_lin_vel = 0.3
        lin_vel = ((max_lin_vel - min_lin_vel)/3)*action + min_lin_vel if action <=3 else ((min_lin_vel - max_lin_vel)/3)*action + 2*max_lin_vel - min_lin_vel
        lin_vel = round(lin_vel, 4)

        vel_cmd = Twist()
        vel_cmd.linear.x = 0.5 #lin_vel
